...
use crate::setup::setup_agentloop_core;
...
async fn main() -> std::io::Result<()> {
    ...
    let config = match crate::config::load_env_config::load_env_config() { // Assuming this function exists and returns AppConfig
    ...
    // Initialize application state using the public setup_agentloop_core function.
    // This function handles merging internal and external tools (if any).
    // For standalone server, pass empty collections for external tools.
    let app_data = match setup_agentloop_core(
        config.clone(),
        None,
        None
    ).await {
        ...
        std::result::Result::Err(e) => {
            log::error!("FATAL: Failed to initialize AgentLoop core: {}", e);
            return std::result::Result::Err(std::io::Error::new(
                std::io::ErrorKind::Other,
                format!("AgentLoop core initialization failed: {}", e),
            ));
        }
    };
    ...
    actix_web::HttpServer::new(move || {
        actix_web::App::new()
            ...
            .configure(crate::config::routes::configure_internal)
...
// No tests needed for the main binary entry point file itself.
// Integration tests should cover server functionality.
